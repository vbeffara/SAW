...
\uses{lem:disjoint_bridge, lem:BT_le_Z}
  For finitely many widths, $\sum_T B_{T+1}^x \leq Z(x)$.
  Uses the disjoint bridge injection.
  \textbf{Status: sorry.} Requires showing the disjoint
  injection across widths.
\end{lemma}
...
\uses{lem:bridge_sum_le_Z, thm:bridge_lower, thm:harmonic_lower}
  ...
  then $Z(x_c) = \infty$.
  \textbf{Status: sorry.} Depends on bridge\_sum\_le\_Z and
  origin\_bridge\_lower\_bound.  
\end{theorem}
...
\begin{itemize}
  \item \texttt{Z\_xc\_diverges} (SAWBridgeFix.lean): $Z(x_c) = \infty$.
    Requires \texttt{origin\_bridge\_lower\_bound} and the disjoint
    bridge injection (\texttt{bridge\_sum\_le\_Z} in SAWHWBridge.lean).
  \item \texttt{bridge\_sum\_le\_Z} (SAWHWBridge.lean):
    $\sum_T B_{T+1}^x \leq Z(x)$. Requires showing the disjoint injection
    of bridges of different widths into SAWs preserves weights.
\end{itemize}
