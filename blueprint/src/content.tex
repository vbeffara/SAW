...
\uses{thm:bridge_in_strip, thm:strip_identity_abs, def:bridge_partition}
  For any finite set $F$ of origin bridges of width $T$:
  $\sum_{b \in F} x_c^{\ell(b)} \leq 1$.
  The proof maps $F$ injectively to $\text{StripSAW\_B}(T,L)$ via
  \texttt{originBridgeToStripB}, then uses $B_{T,L} \leq 1$.
  \textbf{Status: sorry.} Requires formalizing the injection and the
  sum-vs-tsum inequality.
\end{theorem}
...
\uses{def:bridge_decomp, def:bridge_partition, thm:walk_max_x, thm:saw_x_bound}
  ...
  (so that the tsum gives the true value). The factor~$2$ accounts for the
  two choices of first vertex from the starting mid-edge.
