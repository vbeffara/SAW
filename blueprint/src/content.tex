...
\subsection{Bridge-to-SAW injection (new)}

The following infrastructure was added in SAWStripIdentity.lean to
connect bridge partition functions to SAW counts.

\begin{lemma}[Bridge path rewriting]\label{lem:path_cast}
\lean{pathCastStart}
\leanok
  A path starting from $v_1 = v_2$ can be recast as a path from $v_2$.
...
\begin{lemma}[Bridge partial sum bounded by $Z(x)$]\label{lem:bridge_sum_le_Z}
\lean{origin_bridge_finite_sum_le_Z}
\leanok
\uses{lem:bridge_count_le}
  For any finite set of origin bridges, the total weight
  $\sum_{b \in F} x^{\ell(b)}$ is at most $Z(x)$.
\end{lemma}
\begin{proof}
\leanok
\uses{lem:bridge_count_le}
  Group bridges by length, bound each group by $c_n \cdot x^n$, then
  bound the finite sum by the tsum $Z(x)$.
\end{proof}

\begin{lemma}[$B_T^x \leq Z(x)$]\label{lem:BT_le_Z}
\lean{origin_bridge_partition_le_Z}
\leanok
\uses{lem:bridge_sum_le_Z}
...
\uses{lem:BT_le_Z, thm:bridge_lower, thm:harmonic_lower}
  If $B_T^{x_c} \geq c/T$ for some $c > 0$ (origin\_bridge\_lower\_bound),
  then $Z(x_c) = \infty$.
  The proof reduces to showing $\sum_T B_{T+1}$ is summable (given $Z$
  summable), which requires the disjoint injection of bridges of different
  widths into SAWs.
  \textbf{Status: one sorry} (the disjoint bridge summability).
\end{theorem}

\subsection{Remaining sorry gaps}

The following sorry-containing declarations are the key remaining gaps
in the complete formalization of the Hammersley--Welsh argument:

\begin{enumerate}
  \item \texttt{strip\_identity\_concrete} (Lemma~2, SAWFiniteStrip.lean):
    The concrete strip identity $1 = c_\alpha A + B + c_\varepsilon E$.
    Requires formalizing the summing argument for the vertex relation
    over vertices in $S_{T,L}$.
  \item \texttt{origin\_bridge\_upper\_bound} (SAWBridgeFix.lean):
    $B_T^{x_c} \leq 1$. Follows from the strip identity.
  \item \texttt{origin\_bridge\_lower\_bound} (SAWBridgeFix.lean):
    $\exists c > 0, B_T^{x_c} \geq c/T$. Follows from the strip identity
    combined with the quadratic recurrence.
  \item \texttt{bridge\_decomposition\_injection\_proof} (SAWHWDecomp.lean):
    The core counting inequality from the Hammersley--Welsh bridge
    decomposition. Requires formalizing the recursive bridge extraction
    algorithm, its injectivity, and the weight bound.
  \item Disjoint bridge summability in
    \texttt{Z\_xc\_diverges\_from\_lower\_bound} (SAWStripIdentity.lean):
    Bridges of different widths are disjoint SAWs, so $\sum_T B_T \leq Z(x_c)$.
\end{enumerate}
