...
\lean{bridge_partition_scaling}
\leanok
\uses{thm:bridge_bound, thm:bridge_length_ge_width, def:bridge_partition}
...
\leanok
  ...
  This standard combinatorial identity encodes the bijection between
...
\uses{def:bridge_decomp, def:bridge_partition}
  ...
  \textbf{Status: sorry.} This is the key combinatorial gap.
\end{theorem}
...
\uses{thm:bridge_decomp_inj, thm:subset_product, thm:bridge_decay}
  ...
  \textbf{Status: sorry (depends on bridge\_decomposition\_injection).}
\end{theorem}
...
\uses{thm:hw_summable}
  ...
  \textbf{Status: sorry (via hw\_partial\_sum\_bound).}
\end{theorem}
