...
`polygon_umlaufsatz_final` (`RequestProject/SAWUmlaufJordanInduction.lean`) — the planar Umlaufsatz for simple, cyclically non-degenerate closed polygons — is now proved with no `sorry` anywhere in its dependency chain. `#print axioms` reports only `propext`, `Classical.choice`, `Quot.sound`. The same holds for the two hex-lattice forms the SAW development consumes: `hex_signed_turn_eq_six_sign_shoelace` (`RequestProject/SAWUmlaufSignedArea.lean`) and the top-level turning-number theorem `hex_closed_trail_turning_number` (`RequestProject/SAWUmlaufGaussBonnet.lean`).

### The last gap, and how it closed

The remaining live `sorry` was `empty_branch_base_blocked_lift` in `RequestProject/SAWUmlaufPolyMeisters.lean`: in the *empty* branch of the Meisters ear search, the open corner triangle at the extreme apex `b` of `V.rotate r = a :: b :: c :: rest` is empty, but some vertex `w ∈ rest` lies on the base segment `[a, c]`. Then the clip `a :: c :: rest` is not a simple polygon and the clip route is unavailable. It is now proved by cutting `V` along the chord `b–w` instead, mirroring the interior branch step for step. Three pieces made that possible:

- `RequestProject/SAWUmlaufConeStrict.lean` (new, sorry-free) — the strict corner *cone* `HexArea.inConeStrict`, now the third clause of `InteriorChord` in place of "strictly inside the corner triangle". A vertex on the open base lies in the cone but not in the triangle, which is exactly the blocked configuration.
- Generalized interior-split bricks (`interior_split_simple`, `interior_split_select`, `interior_split_nondeg_left/right`, `interior_split_one_nondeg`, `interior_flat_seam_data*`): they now take the two corner cross conditions and the chord clearance instead of `inTriangleStrict` and the distance-maximality hypothesis.
- `RequestProject/SAWUmlaufBaseBlocked.lean` (new, sorry-free) — `base_vertex_cross_facts`, `base_vertex_inConeStrict`, and `base_chord_is_diagonal`, the blocked-base analogue of `interior_chord_is_diagonal` (the chord `b–w` meets no non-incident closed edge).

Earlier in the same sequence, `RequestProject/SAWUmlaufEmptyBranchLift.lean` (sorry-free) closed the flat-clip residue with `empty_branch_lift_of_clip_data`.

### Other cleanup

- The point-in-polygon dichotomy was still stated as an unproved topological input upstream. It is proved unconditionally downstream as `polygon_ptWind_dichotomy_final`, so the upstream sorry'd copy was removed (its statement kept in a comment) and the five already-superseded lemmas that consumed it now take the relevant `PolyDichotomy` instance as an explicit hypothesis; all are sorry-free.
- Added `RequestProject/SAWUmlaufTriangleExample.lean`: all three hypotheses of the main theorem verified for the concrete triangle `0, 1, i`, giving an unconditional instance of its conclusion. This confirms the statement is not vacuous. It is imported from `RequestProject/SAWFinal.lean`.

### What remains

One `sorry` is left in the Umlauf files, `vertex_escape_joinedIn_arbitrarily_far_one_diag` in `RequestProject/SAWUmlaufPolyEscape.lean`. It is **not** on the live route and is not needed by the theorem; its docstring now says so explicitly and marks it as banked material for a possible future direct treatment of `clipped_ear_escape_walk`. Three further textual `sorry` occurrences in Umlauf files sit inside `/- ... -/` blocks recording superseded statements. `sorry`s elsewhere in the project (strip observable, recurrence proof, …) lie outside the Umlaufsatz and were not touched, as requested.

Every new file is imported (`SAWUmlaufConeStrict` by `SAWUmlaufChordCorner` and `SAWUmlaufBaseBlocked`; `SAWUmlaufBaseBlocked` and `SAWUmlaufEmptyBranchLift` by `SAWUmlaufPolyMeisters`; `SAWUmlaufTriangleExample` by `SAWFinal`). The full project builds. `PROOF_STATUS.md` and `ARISTOTLE_SUMMARY.md` are updated, and everything is committed and pushed.

# Summary of changes for the current run

This round worked exclusively on the Umlaufsatz, as requested, and **completed
it**.  `polygon_umlaufsatz_final`
(`RequestProject/SAWUmlaufJordanInduction.lean`) and the hex form
`hex_signed_turn_eq_six_sign_shoelace`
(`RequestProject/SAWUmlaufSignedArea.lean`) are now `sorry`-free and depend only
on `propext`, `Classical.choice`, `Quot.sound`.  The project builds and all work
is committed and pushed.

## 1. The last live gap: `empty_branch_base_blocked_lift`

In the *empty* branch of the Meisters ear search the convex apex `b` of
`V.rotate r = a :: b :: c :: rest` has an empty open corner triangle, but some
vertex `w ∈ rest` lies on the base segment `[a, c]`; then the clip
`a :: c :: rest` is not simple and the clip route is unavailable.  The lemma is
now proved by cutting `V` along the chord `b–w` instead, mirroring
`meisters_reduction_interior2` step for step.

Three preparations made that possible:

* **`RequestProject/SAWUmlaufConeStrict.lean`** (`sorry`-free): the strict corner
  cone `HexArea.inConeStrict p u n v`, which now forms the third clause of
  `InteriorChord` in place of `inTriangleStrict`.  A vertex on the *open base* of
  the corner lies in the cone but not in the open triangle — exactly the
  blocked-base configuration.
* **Generalized interior-split bricks**: `interior_split_nondeg_left/right`,
  `interior_split_one_nondeg`, `interior_split_simple`, `interior_split_select`,
  `interior_flat_seam_data*` now take the two corner cross conditions
  `cross (b-a) (w-a) ≠ 0`, `cross (c-b) (w-b) ≠ 0` and the chord clearance,
  instead of `inTriangleStrict a b c w` and the distance maximality `hwmax`.
* **`RequestProject/SAWUmlaufBaseBlocked.lean`** (new, `sorry`-free):
  `base_vertex_cross_facts` (the three corner cross products at a point
  `w - a = s (c - a)`, `0 < s < 1`), `base_vertex_inConeStrict`, and
  `base_chord_is_diagonal` — the blocked-base analogue of
  `interior_chord_is_diagonal`: the chord `b–w` meets no closed edge of the
  polygon not incident to `b` or `w`.  The maximality hypothesis of the interior
  case is replaced by the emptiness of the open corner triangle.

Closed in the same sequence: **`RequestProject/SAWUmlaufEmptyBranchLift.lean`**
(`sorry`-free) with `empty_branch_boundary_lift_weak`,
`empty_branch_interior_lift_flat` and `empty_branch_lift_of_clip_data`.

## 2. The sorry'd `polygon_ptWind_dichotomy` retired

The point-in-polygon dichotomy was still stated as an unproved topological input
in `RequestProject/SAWUmlaufJordanDichotomy.lean`.  It is *proved*
...
upstream sorry'd copy is removed (its statement retained in a comment), and the
already-superseded lemmas that consumed it — `ear_interior_ptWind_eq`,
`ear_interior_clip_ptWind_zero`, `ear_interior_ptWind_ne_zero_via_clip`,
`ear_interior_ptWind_ne_zero_of_rotation`, `ear_interior_ptWind_ne_zero` — now
take the relevant `PolyDichotomy` instance as an explicit hypothesis and are
`sorry`-free.

## 3. Remaining `sorry`s in the Umlaufsatz files

Exactly one, and it is neither on the live route nor needed by the theorem:
`vertex_escape_joinedIn_arbitrarily_far_one_diag`
(`RequestProject/SAWUmlaufPolyEscape.lean`), explicitly marked as banked material
for a possible future direct treatment of `clipped_ear_escape_walk`.  The three
other textual `sorry` occurrences in Umlauf files sit inside `/- ... -/` blocks
recording superseded statements.  `sorry`s elsewhere in the project (SAW strip
observable, recurrence proof, …) lie outside the Umlaufsatz and were not touched.

All new files are imported: `SAWUmlaufConeStrict` by `SAWUmlaufChordCorner` and
`SAWUmlaufBaseBlocked`; `SAWUmlaufBaseBlocked` and `SAWUmlaufEmptyBranchLift` by
...
**1. A false hypothesis shape, now formally disproved.**
Seventeen declarations of the chord branch in `RequestProject/SAWUmlaufPolygon.lean` described the cut `W[0]–W[k]` only by edge-disjointness ("the chord segment is disjoint from every non-incident edge of `W`"). That does not force the chord to run through the *inside* of the polygon: for a dart (non-convex quadrilateral) every edge touches one of the two chord endpoints, so the hypothesis holds vacuously for the *exterior* chord, and the reflex vertex then lies strictly inside the triangle cut off by it.

The new file `RequestProject/SAWUmlaufDartCounterexample.lean` (imported by `RequestProject.SAWFinal`) makes this precise and is **`sorry`-free**: with the dart `q = -2, x = -i, p = 2, r = -4i` cut at `k = 2` it proves
- `chord_ear_empty_other_general_false`,
- `chord_ear_other_ptWind_zero_general_false`,
- `vertex_escape_walk_core_general_false`,

each depending only on `propext`, `Classical.choice`, `Quot.sound`. Note that `chord_ear_empty_other` had previously been *derived* from two `sorry`ed lemmas, so this was a live unsoundness, not just an over-general statement.

**2. The fix.** A new predicate `InteriorChord W u v` records the missing diagonal-validity data: the rooted endpoint `u = W.head` is an extreme vertex of `W` (no triangle of vertices contains it strictly, so the corner there is convex) and the far endpoint `v` lies strictly inside the corner triangle at `u`. It was threaded through the whole branch and is **discharged at the sole call site**, Meisters' interior branch `meisters_reduction_interior2`, where `u = b` is the extreme corner apex and `v = w` is the deepest vertex inside the corner triangle. The counterexample file also proves `dart_chord_not_interiorChord`, confirming the strengthened hypothesis really excludes the bad configuration.

**3. A new reduction of the branch's Jordan residue.** New `sorry`-free file `RequestProject/SAWUmlaufChordArcWind.lean` (`SegAvoids`, `isChain_of_forall_mem`, `ptWind_eq_head_of_isChain`, `ptWind_const_of_isChain`, `ptWind_zero_of_isChain_witness`, `ptWind_zero_of_isChain_hull_witness`), consumed in `SAWUmlaufPolygon.lean` by the new proved bricks `isChain_closedEdges_self`, four chord-piece membership lemmas, `chordPiece_step_segAvoids`, `chordPiece_other_arc_chain` and `chord_ear_other_ptWind_zero_of_witness`. The vertices of `W` off the piece `P` are joined consecutively by `W`-edges disjoint from every cycle edge of `P`, so the winding `ptWind · P` is constant on that whole set. Consequently `chord_ear_other_ptWind_zero` now splits into a **proved** case (some such vertex lies outside `convexHull P`) and the genuine residue (all of them lie inside it) — the remaining Jordan content now needs only *one* witness vertex.

**Status.** The full `RequestProject.SAWFinal` target builds (8168 jobs). Everything added this round is `sorry`-free; the live Umlaufsatz `sorry`s are the same seven polygon-Jordan / two-ears residues as before (`empty_branch_boundary_lift` ×2, `vertex_escape_joinedIn_arbitrarily_far_one_diag`, `clipped_ear_escape_walk`, `chord_ear_lift`, `interior_lift_via_piece`, `empty_branch_bad_lift`), all in `RequestProject/SAWUmlaufPolygon.lean` and all now carrying corrected hypotheses. Both new files are imported on the live route to the main theorem. `PROOF_STATUS.md` and `ARISTOTLE_SUMMARY.md` document the counterexample, the fix and the new reduction; all work is committed and pushed.

# Summary of changes for the current run

This round worked only on the Umlaufsatz.  It found and fixed a **soundness bug**
in the chord-splitting branch, backed by a fully verified counterexample, and
then reduced the branch's Jordan residue.

## 1. Formal counterexample: edge-disjointness does not make a chord interior

New, **`sorry`-free** file `RequestProject/SAWUmlaufDartCounterexample.lean`
(imported by `RequestProject.SAWFinal`).  Using the dart
`q = -2, x = -i, p = 2, r = -4i` cut at `k = 2`, it disproves the three
hypothesis shapes that the chord branch had been using:

* `chord_ear_empty_other_general_false`,
* `chord_ear_other_ptWind_zero_general_false`,
* `vertex_escape_walk_core_general_false`.

For a quadrilateral every edge touches one of the two chord endpoints, so the
old hypothesis "`u–v` is disjoint from every non-incident edge" holds *vacuously*
for the **exterior** chord of a dart, and the reflex vertex then lies strictly
inside the triangle cut off by it.  All three theorems use only the standard
axioms.

## 2. The fix: a new `InteriorChord` hypothesis, discharged at the call site

`InteriorChord W u v` (new definition in `SAWUmlaufPolygon.lean`) says the chord
*enters the polygon at its rooted endpoint*: `u = W.head`, `u` is an extreme
vertex of `W` (no triangle of vertices contains it strictly), and the far
endpoint `v` lies strictly inside the corner triangle at `u`.  It was threaded
through the seventeen affected declarations and is **discharged** in Meisters'
interior branch `meisters_reduction_interior2`, where `u = b` is the extreme
corner apex and `v = w` is the deepest vertex inside the corner triangle.  The
whole project still builds.

## 3. New reduction: the other-piece winding needs only one witness

New `sorry`-free file `RequestProject/SAWUmlaufChordArcWind.lean`
(`HexArea.SegAvoids`, `isChain_of_forall_mem`, `ptWind_eq_head_of_isChain`,
`ptWind_const_of_isChain`, `ptWind_zero_of_isChain_witness`,
`ptWind_zero_of_isChain_hull_witness`), consumed in `SAWUmlaufPolygon.lean` by
the new proved bricks `isChain_closedEdges_self`, the four chord-piece membership
lemmas, `chordPiece_step_segAvoids`, `chordPiece_other_arc_chain` and
`chord_ear_other_ptWind_zero_of_witness`.

The vertices of `W` off the piece `P` are joined consecutively by `W`-edges that
are disjoint from every cycle edge of `P`, so `ptWind · P` is constant on them.
`chord_ear_other_ptWind_zero` now splits into a **proved** case (some such vertex
lies outside `convexHull P`) and the genuine residue (all of them lie inside it).

## 4. Status

`RequestProject.SAWFinal` builds (8168 jobs).  The live Umlaufsatz `sorry`s are
the same seven polygon-Jordan / two-ears residues as before, all in
`RequestProject/SAWUmlaufPolygon.lean`; everything added this round is
`sorry`-free.  `PROOF_STATUS.md` records the details.
...
**New durable, machine-checked Lean this round** — two `sorry`-free, reusable escape-walk bricks added to `RequestProject/SAWUmlaufEscapeHelpers.lean` (already imported by `SAWUmlaufPolygon.lean`, so they are linked, not a dead branch):

- `exists_far_not_mem_convexHull_nonneg` — strengthens the existing far-point lemma so the escape parameter `T` is guaranteed `≥ 0`; the far point lies on the *forward* ray `{x + T•d : T ≥ 0}`.
- `exists_escape_walk_of_ray` — a fully generic reduction: given a nonzero direction `d` such that the escape relation `R` holds from `x` to every forward-ray point `x + T•d` (`T ≥ 0`), it produces the required `R`-walk `x :: zs` whose endpoint lies outside `convexHull ℝ S`. This is exactly the shape of both remaining escape-walk residues (`clipped_ear_escape_walk`, `chord_ear_other_escape_walk`), reducing them — whenever a straight forward ray escapes — to the sharper task of finding an escaping ray direction. It is banked as preparation rather than consumed unsoundly, since a non-convex "pocket" point can still require a genuine polyline.

**Attempts on the deep gaps.** I made genuine high-effort automated attempts on the linchpin `chord_ear_lift` (pointing at the full set of already-banked bricks: `chord_consec_triple_lift`, `chord_ear_empty_other`, `chordLeft/chordRight_interior_ear_extract`, `chordLeft_ear_tl_neighbours`, `rotate_drop3_neighbours`, `polyCycNondeg_interior_corner`) and on the `interior_lift_via_piece` triangle/flat residual. Neither closed in a single search, confirming that the remaining seam-corner non-degeneracy, cyclic-neighbour identification, and two-ears content is genuinely multi-round work.

**Honest status.** The 7 live Umlaufsatz `sorry`s are unchanged in count and remain the genuine polygon-Jordan / two-ears core absent from Mathlib: the two escape-walk residues (now targeting `convexHull W`), the two boundary "spike" subcases, `chord_ear_lift`, the `interior_lift_via_piece` residual, and `empty_branch_bad_lift`. The general planar `polygon_umlaufsatz` and its ear-clipping induction, plus the analytic/signed-area toolchain and the top statements, remain in place; the content is concentrated in these Jordan bricks. Partial progress is preserved as building Lean with honest, sharply-isolated `sorry`s (nothing lost), and this round adds proven, reusable geometry that future rounds build on. The round is recorded at the top of `PROOF_STATUS.md`.
...
The two remaining point-in-polygon atoms of the Umlaufsatz reduce (via the already-proved walk-invariance tool `HexArea.ptWind_zero_of_walk_to_not_hull`) to exhibiting an edge-avoiding polyline from the forbidden point out past the convex hull. This round I separated the *combinatorial* part of those two escape-walk residues (which edges must be avoided) from the *topological* part (routing through the exterior of the whole polygon), by proving two new fully `sorry`-free reusable lemmas in `RequestProject/SAWUmlaufPolygon.lean` and immediately consuming each:

- `chordPiece_cycleEdge_or_diag`: every closed cycle edge of a chord piece `P = chordLeft/chordRight W k` has both endpoints in `P` and is either an honest closed edge of `W` or the cut diagonal `u–v`. It is consumed inside `chord_ear_other_escape_walk`, whose `suffices` step now reduces "avoid every edge of `P`" to "avoid `W`'s non-`x`-incident edges plus the diagonal" (using that `x ∉ P`).
- `clippedPiece_cycleEdge_classify`: every closed cycle edge of the ear-clipped polygon `a' :: c' :: tlP` is the ear base `a'–c'`, or a closed edge of `W`, or the diagonal `u–v` (clipping only swaps the two ear sides for the base; the rest are edges of the rotated piece, classified by the previous lemma). It is consumed inside `clipped_ear_escape_walk`, whose `suffices` step reduces to avoiding the ear base, `W`'s edges not carrying the removed tip `b'` (nor `x`), and the diagonal — deliberately leaving the two ear sides free, which is what allows the walk to escape the ear triangle.

Net effect: the two escape-walk residues no longer carry opaque `sorry`s; each now carries only the strictly cleaner, honest *reduced* exterior-walk goal (route an edge-avoiding polyline through the polygon's exterior out past the hull), backed by two proved, reusable classification lemmas. Both new lemmas were verified to use only the standard axioms (`propext`, `Classical.choice`, `Quot.sound`). I judged that extracting the two reduced goals into a single general "exterior is polyline-connected to infinity" brick would not be honest, since such a brick is only true under an "the point lies in the unbounded component" hypothesis that cannot be supplied generically; keeping each residue in its fully-hypothesised context avoids introducing a lemma that is false in general.

The remaining live Umlaufsatz `sorry`s (7, all in `RequestProject/SAWUmlaufPolygon.lean`, each honest and consumed downstream, none dead branches) are: the two reduced exterior-walk goals (the genuine Jordan-complement path-connectivity content), the two boundary-spike subcases, the interior ear-lift brick `chord_ear_lift`, the `interior_lift_via_piece` triangle/flat residual, and `empty_branch_bad_lift`. Progress recorded in `PROOF_STATUS.md`.

Files touched (relative): `RequestProject/SAWUmlaufPolygon.lean` (two new proved lemmas plus the two consuming `suffices` reductions), `PROOF_STATUS.md` (round note).
...
What I did this round:
- Mapped the exact live dependency structure of `polygon_umlaufsatz`. The proof reduces (via the sorry-free strong-induction wrapper `exists_empty_corner_avoiding_aux2` → `meisters_reduction2`) to three live gaps: `meisters_reduction_interior2` (interior diagonal split branch), `empty_branch_bad_lift` (bad-diagonal branch), and the two "spike" subcases inside `empty_branch_boundary_lift`. The quadrilateral base case `meisters_reduction_quad2` is already sorry-free. The two preparation keystones `chord_ear_empty_other` and `chord_ear_lift` are the shared geometric residue of those branches.
- Confirmed that every one of these gaps funnels into a single irreducible fact: the planar point-in-polygon / Jordan-curve separation lemma `chord_ear_empty_other` (a convex ear triangle of one chord piece contains no vertex of the other piece, given a valid splitting diagonal). I additionally verified there is no combinatorial shortcut around it — e.g. the natural "triangle base case" for a chord piece is not expressible as `EmptyCornerData2` (that predicate forces ≥ 4 vertices) and the direct triangle-ear lift itself reduces back to the same separation fact.
- Made two fresh high-effort automated proof attempts — on the isolated keystone `chord_ear_empty_other`, and on the central branch `meisters_reduction_interior2` (wired through the banked split/lift bricks and `chord_ear_lift`). Both confirm the blocker: the residue is genuine winding-number / point-in-polygon region separation, which is absent from Mathlib and from the project.

No sorries were eliminated: the remaining Umlaufsatz gap is a single research-level plane-topology result (polygon Jordan separation) requiring new point-in-polygon infrastructure that cannot be built soundly in one round. Rather than add speculative scaffolding that risks regressions on the large (5158-line) file, I preserved the extensive existing progress intact — every supporting file is imported and part of the build target, each remaining gap is documented and linked to the branch that consumes it, and no dead branches were introduced.

# Summary of changes for run f5f496f3-35fa-42ea-9b29-9ad24e786879
Continued work exclusively on the top-priority discrete Hopf Umlaufsatz (`polygon_umlaufsatz` in `RequestProject/SAWUmlaufPolygon.lean`). The library remains healthy: it builds end-to-end (8128 jobs through `RequestProject/SAWFinal.lean`) with only the standard axioms plus `sorryAx`, no regressions, and the working tree is fully committed. All project files are part of the build target (the lakefile uses a `RequestProject.+` glob), so nothing is orphaned, and each remaining gap is documented and linked to the branch that will consume it (no dead branches).

Verified findings this round:
- The main theorem `polygon_umlaufsatz` still depends on `sorryAx` through a small set of interlocking gaps, all of which I traced to genuine Jordan-curve / two-ears content. The single geometric keystone everything reduces to is `chord_ear_empty_other` (a point-in-polygon separation fact: a vertex of one chord piece cannot lie strictly inside a convex ear triangle of the other piece).
- I took a fresh high-effort attempt at `chord_ear_empty_other` with a new arc-crossing argument sketch and the relevant cross-product/segment bricks; it did not close, confirming the gap needs a winding-number / region-membership predicate that is absent from Mathlib and the project.
- I took a fresh high-effort attempt at the best-supported combinatorial branch `meisters_reduction_interior2`, wiring it through the proved split bricks (`interior_split_select`, `mem_closedEdges_rotate`, `IsCycEdge_rotate`, `forbidden_lands_in_chord`, `chordLeft/Right_cut_isCycEdge`, `chord_tip_ne_other`) and the `chord_ear_lift` keystone. This got the assembly structure right but exposed three genuine residual obstructions that must be resolved before the branch can close: (1) the recursed chord piece can be a triangle, where the inductive invariant `EmptyCornerData2` is inapplicable (it needs ≥ 4 vertices); (2) only one of the two split pieces is guaranteed non-degenerate, so when the forbidden edge lands in the non-degenerate piece the recursion is forced onto the degenerate one and the flat seam vertex must first be removed; (3) `chord_ear_lift` itself still needs both the keystone and a seam turn-transfer that coincides with the two-ears "spike" residue.

No sorries were eliminated: the remaining Umlaufsatz gaps are research-level plane-topology results requiring new infrastructure that could not be built soundly in this round. Rather than introduce unsound scaffolding, I preserved the existing extensively-documented partial state so future rounds can build on it. The concrete next step is to develop a minimal point-in-polygon / winding-number separation predicate, which would unlock `chord_ear_empty_other` and, in turn, `chord_ear_lift` and the interior/bad-diagonal split branches.
