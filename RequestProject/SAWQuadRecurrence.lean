...
lemma quadratic_lower_step {α x y : ℝ} (hα : 0 < α) (hx_pos : 0 < x)
  ...
  have h_mul : x * (1 + α * y) ≥ y := by
    nlinarith [ mul_le_mul_of_nonneg_left hy_pos.le hα.le, mul_le_mul_of_nonneg_left hy_pos.le hy_pos.le, sq_nonneg ( x - y ), sq_nonneg ( x - 1 ), sq_nonneg ( y - 1 ) ] ;
...
theorem quadratic_recurrence_lower_bound
  ...
  · contrapose! hB_rec;
    exact ⟨ k, hk, by nlinarith [ hB_pos k hk, hB_pos ( k + 1 ) ( by linarith ), mul_inv_cancel₀ ( ne_of_gt hα ), mul_pos hα ( hB_pos ( k + 1 ) ( by linarith ) ) ] ⟩;
  · contrapose! h;
    nlinarith [ inv_pos.2 hα, mul_inv_cancel₀ hα.ne', hB_pos k hk, hB_pos ( k + 1 ) ( by linarith ), hB_rec k hk, mul_pos hα ( hB_pos ( k + 1 ) ( by linarith ) ) ]
...
theorem quadratic_recurrence_exists_lower
  ...
  · exact fun T a => quadratic_recurrence_lower_bound hα hB_pos hB_rec T a
