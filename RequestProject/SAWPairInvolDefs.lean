/-
# Pair Involution: definitions and basic structure

This file holds the *definitional* part of the pair involution that used to
live at the top of `RequestProject.SAWPairCancellation`:
`pairExitIdx`, `pairInner`, `pairPrefix`, `pairDecomp`, `pairInvolWalk`,
`pairInvol` and `pairInvol_length`.

It was split off so that the winding-decomposition files
(`SAWWindingDecomp`, `SAWPairWindingProof`) and the geometric route to
`pair_winding_relation` can be developed *upstream* of
`SAWPairCancellation`, which now consumes them.  Nothing else changed.
-/
